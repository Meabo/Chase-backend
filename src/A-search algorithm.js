class AstarAlgorithm {
  ...
  calculateHeuristicValue(x, y, destination) {
    let hypothenus = Math.sqrt(
      (x - destination.x) ** 2 + (y - destination.y) ** 2
    );
  ...
  update_neightbor(current_x, current_y, prev_x, prev_y, src, dest, value) {
    if (this.isValid(current_x, current_y) === true) {
      ...
      if (this.isDestination(current_x, current_y, dest)) {
        ...
        this.closed_list[current_x][current_y] === false &&
        ...
        if (
          this.cell_details[current_x][current_y].get_f() ===
            Number.MAX_SAFE_INTEGER ||
          ...
          // We update the cell
          this.cell_details[current_x][current_y] = new Cell(
            prev_x,
            prev_y,
            new_g,
            new_h,
  ...
  visit_neighbors(x, y, src, dest) {
    ...
    Promise.all([
      this.update_neightbor(x - 1, y, x, y, src, dest, 1.0),
      this.update_neightbor(x + 1, y, x, y, src, dest, 1.0),
      this.update_neightbor(x, y - 1, x, y, src, dest, 1.0),
      this.update_neightbor(x, y + 1, x, y, src, dest, 1.0),
      this.update_neightbor(x - 1, y + 1, x, y, src, dest, Math.sqrt(2)),
      this.update_neightbor(x + 1, y + 1, x, y, src, dest, Math.sqrt(2)),
      this.update_neightbor(x - 1, y - 1, x, y, src, dest, Math.sqrt(2)),
      this.update_neightbor(x + 1, y - 1, x, y, src, dest, Math.sqrt(2))
  ...
  require_checking(src, dest) {
    ...
    if (
      this.isUnBlocked(src.x, src.y) === false ||
  ...
  AstarSearch(source, destination) {
    ...
    if (this.require_checking(source, destination) === false) return;

    this.cell_details = this.initArray();
    this.closed_list = this.initArrayClosed();

    // Initialisation of the source
    this.cell_details[source.x][source.y] = new Cell(
      source.x,
      source.y,
      0.0,
      0.0,
    ...
    while (this.open_list.size > 0) {
      ...
      let result = this.visit_neighbors(
        first_entry_x,
        first_entry_y,
        source,
        destination
